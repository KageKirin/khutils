#ifndef KHUTILS_SJIS_TO_UTF8_HPP_INC
#define KHUTILS_SJIS_TO_UTF8_HPP_INC

#if defined(KHUTILS_SJIS_TO_UTF8_IMPL)

//# SHIFTJIS.TXT
//# Date: 2015-12-02 23:52:00 GMT [KW]
//# テつゥ 2015 Unicodeテつョ, Inc.
...
namespace khutils
{
	namespace sjis_to_utf8
	{
		...
		struct sjisutf8_elem
		{
			uint16_t sjis_code;
			uint16_t utf8_code;
		};

		static const std::vector<sjisutf8_elem> sjis_utf8_map{
		  {0x20, 0x0020},	  //# SPACE
		  ...
		  {0xDF, 0xFF9F},	  //# HALFWIDTH KATAKANA SEMI - VOICED SOUND MARK{0xE040,
							   // 0x6F3E},	//# <CJK>
		  {0xE041, 0x6F13},	//# <CJK>
		  {0xE042, 0x6EF7},	//# <CJK>
		  {0xE043, 0x6F86},	//# <CJK>
		...
		// check lambdas
		auto hasSjisCode = [](const auto& _elem, uint16_t code) { return _elem.sjis_code == code; };

		auto hasUtf8Code = [](const auto& _elem, uint16_t code) { return _elem.utf8_code == code; };

		bool validSjisCode(uint16_t code)
		{
			return std::any_of(sjis_utf8_map.begin(), sjis_utf8_map.end(), std::bind(hasSjisCode, std::placeholders::_1, code));
		}

		bool validUtf8Code(uint16_t code)
		{
			return std::any_of(sjis_utf8_map.begin(), sjis_utf8_map.end(), std::bind(hasSjisCode, std::placeholders::_1, code));
		}

		// accessor functions
		uint16_t sjisToUtf8(uint16_t code)
		{
			auto foundIt
			  = std::find_if(sjis_utf8_map.begin(), sjis_utf8_map.end(), std::bind(hasSjisCode, std::placeholders::_1, code));
			if (foundIt != sjis_utf8_map.end())
			{
				return foundIt->utf8_code;
			}

			return 0x20;
		}

		uint16_t utf8ToSjis(uint16_t code)
		{
			auto foundIt
			  = std::find_if(sjis_utf8_map.begin(), sjis_utf8_map.end(), std::bind(hasUtf8Code, std::placeholders::_1, code));
			if (foundIt != sjis_utf8_map.end())
			{
				return foundIt->sjis_code;
			}
...
#endif	// defined (KHUTILS_SJIS_TO_UTF8_IMPL)

#endif	// !KHUTILS_SJIS_TO_UTF8_HPP_INC
